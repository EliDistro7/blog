...
// Added onRegenerateSuggestions and onIndustrySuggestions to props
export default function QuickPrompts({ 
  ...
  currentDetection,
  insights,
  onRegenerateSuggestions, // new prop
  onIndustrySuggestions   // new prop
}) {
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            ...
            {/* Add Regenerate Suggestions button if prop is provided */}
            {typeof onRegenerateSuggestions === 'function' && (
              <div className="flex justify-center mt-2">
                <motion.button
                  onClick={onRegenerateSuggestions}
                  className="text-xs text-yellow-400 hover:text-yellow-300 transition-colors flex items-center gap-1 border border-yellow-400 rounded-full px-3 py-1"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  <Sparkles size={14} />
            ...
            {/* Add Industry Suggestions button if prop is provided */}
            {typeof onIndustrySuggestions === 'function' && (
              <div className="flex justify-center mt-2">
                <motion.button
                  onClick={() => onIndustrySuggestions(serviceContext?.industry)}
                  className="text-xs text-cyan-400 hover:text-cyan-300 transition-colors flex items-center gap-1 border border-cyan-400 rounded-full px-3 py-1"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  <Brain size={14} />
